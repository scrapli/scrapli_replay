...
def pylint(session):
    ...
    """
    session.install(*_get_install_test_args())
    session.run("python", "-m", "pylint", "scrapli_replay/")
...
def mypy(session):
    ...
    session.install(f"pytest{DEV_REQUIREMENTS['pytest']}")
    session.install(".")
    session.run("python", "-m", "mypy", "--strict", "scrapli_replay/")
...
def darglint(session):
    ...
    for file in Path("scrapli_replay").rglob("*.py"):
